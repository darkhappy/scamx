...
if (!function_exists("__c3_error")) {
  function __c3_error($message)
  {
    ...
    if (!headers_sent()) {
      header(
        "X-Codeception-CodeCoverage-Error: " . str_replace("\n", " ", $message),
        true,
        500
      );
    }
...
if (
  !class_exists("\\Codeception\\Codecept") ||
  !function_exists("codecept_is_path_absolute")
) {
  if (file_exists(__DIR__ . "/codecept.phar")) {
    ...
    if (
      stream_resolve_include_path(
        __DIR__ . "/vendor/codeception/codeception/autoload.php"
      )
    ) {
    ...
    __c3_error(
      "Codeception is not loaded. Please check that either PHAR or Composer package can be used"
    );
  }
}
...
if (
  !class_exists("PHP_CodeCoverage") and
...
$configDistFile =
  realpath(__DIR__) . DIRECTORY_SEPARATOR . "codeception.dist.yml";
...
if (!defined("C3_CODECOVERAGE_MEDIATE_STORAGE")) {
  ...
  $requiredMemory = "384M";
  if (
    (substr($memoryLimit, -1) === "M" &&
      (int) $memoryLimit < (int) $requiredMemory) ||
    (substr($memoryLimit, -1) === "K" &&
      (int) $memoryLimit < (int) $requiredMemory * 1024) ||
    (ctype_digit($memoryLimit) &&
      (int) $memoryLimit < (int) $requiredMemory * 1024 * 1024)
  ) {
  ...
  define(
    ...
    Codeception\Configuration::outputDir() . "c3tmp"
  );
  define(
    "C3_CODECOVERAGE_PROJECT_ROOT",
    Codeception\Configuration::projectDir()
  );
  define(
    "C3_CODECOVERAGE_TESTNAME",
    $_SERVER["HTTP_X_CODECEPTION_CODECOVERAGE"]
  );
  ...
  function __c3_factory($filename, $lock = false)
  {
    ...
    if (isset($_SERVER["HTTP_X_CODECEPTION_CODECOVERAGE_SUITE"])) {
      ...
      try {
        $settings = Configuration::suiteSettings(
          $suite,
...
if (!is_dir(C3_CODECOVERAGE_MEDIATE_STORAGE)) {
  if (mkdir(C3_CODECOVERAGE_MEDIATE_STORAGE, 0777, true) === false) {
    __c3_error(
      'Failed to create directory "' . C3_CODECOVERAGE_MEDIATE_STORAGE . '"'
    );
  }
}
...
$path =
  realpath(C3_CODECOVERAGE_MEDIATE_STORAGE) .
  DIRECTORY_SEPARATOR .
  "codecoverage";
...
if ($requestedC3Report) {
  ...
  if (!array_key_exists("HTTP_X_CODECEPTION_CODECOVERAGE_DEBUG", $_SERVER)) {
    register_shutdown_function(function () use ($codeCoverage, $currentReport) {
      $codeCoverage->stop();
      if (!file_exists(dirname($currentReport))) {
      ...
      if ($file === null) {
        file_put_contents(
          $currentReport,
          serialize($existingCodeCoverage),
          LOCK_EX
        );
      } else {
        fseek($file, 0);
